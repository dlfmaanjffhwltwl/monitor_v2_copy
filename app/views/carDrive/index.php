    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header ai-fs">
                <h5 class="modal-title">운행 일지 작성</h5>
            ...
            <div class="modal-body">
                <form id="formReg">
                    <table class="driveRegTable">
                        ...
                        <tr>
                            <td colspan="4">
                                <input type="checkbox" id="reportFaultButton">고장신고</button>
                                <input type="hidden" name="drivereport" id="drivereport" value="N">
                            </td>
                        </tr>
                    </table>
                    <hr>
...
// 고장버튼
// 체크박스 변경 시 이벤트 처리
document.getElementById('reportFaultButton').addEventListener('change', function() {
    // 체크박스의 체크 여부에 따라 값을 변경 (체크되면 "Y", 아니면 "N")
    var drivereportInput = document.getElementById('drivereport'); // 수정된 부분
    drivereportInput.value = this.checked ? 'Y' : 'N';
});
...
function dataTableStart(data = null) {
    var table = $('#dataTable').DataTable({
        ...
        columns: [{ //가져온 데이터 컬럼에 할당
                data: "Drive_key",
                className: "driveKeyTd",
            },
            {
                data: "Drive_date",
            },
            {
                data: "Drive_car_terminal",
                render: function(data, type) {
                    return "<span data-bs-toggle='modal' data-bs-target='#carInfoDetailModal' class='under-l carInfoDetail'>" +
                        carInfoObject[data]["Car_number"] +
                        "</span>"
                }
            },
            {
            ...
            },
            {
                data: "Drive_purpose",
                render: function(data, type) {
                    return "<span data-bs-toggle='modal' data-bs-target='#carDriveDetailModal' class='under-l driveDetail'>" +
                        data +
                        "</span>"
                }
            },
            {
                data: "Drive_significant",
                className: "d-none",
                render: function(data, type) {
                    data = data.replace(/(?:\r\n|\r|\n)/g, '<br />');
                    return data;
...
formReg.addEventListener('submit', async (e) => {
    ...
    if (!forntCheck) {
        ...
        try {
            const response = await fetch('carDrive/addCarDrive', {
                method: 'POST',
                cache: 'no-cache',
                body: payload,
            });
            ...
            console.log(responseData); // 콘솔에 서버 응답을 출력
